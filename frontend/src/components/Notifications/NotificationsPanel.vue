<template>
  <Teleport to="body">
    ...
    >
      <div
        v-if="open"
        class="fixed inset-0 z-[19] bg-black/10 sm:left-[50px]"
        @click="open = false"
      />
    </Transition>

    <div class="pointer-events-none fixed inset-y-0 left-0 right-0 z-20 overflow-hidden sm:left-[50px]">
      <Transition
      ...
      >
        <aside
          v-if="open"
          class="pointer-events-auto absolute inset-y-0 left-0 flex w-full flex-col border-r border-outline-gray-2 bg-surface-base shadow-2xl sm:w-[26rem]"
        >
...
import { Avatar, Badge, Button, ScrollArea, TabButtons, Tooltip } from 'frappe-ui'
