...
import { procedure, acknowledge } from '@/data/procedures'
...
const doc = computed(() => procedure.doc || {})
...
const actions = computed(() => [
  ...
  },
  {
    label: 'Edit',
    icon: 'lucide-pencil',
    condition: () => doc.value.can_edit,
    onClick: () => router.push(`/${route.params.name}/edit`),
  },
])

function load() {
  procedure.fetch({ name: route.params.name, revision: revision.value })
}
...
<template>
  <PageHeader>
    ...
    <div class="flex items-center gap-2">
      <!-- Reading an old revision is legitimate; looking at one by accident is not,
           so the switcher states plainly which one is on screen. -->
      <Select
        v-if="doc.revisions?.length > 1"
        v-model="revision"
        :options="doc.revisions.map((r) => ({ label: `Rev ${r.version}`, value: r.version }))"
      />
      <Dropdown :options="actions.filter((a) => !a.condition || a.condition())">
  ...
  <div class="mx-auto mt-5 w-full max-w-[740px] px-3 pb-24 sm:px-5">
    <!-- Anything not currently in force says so before the first line of text. -->
    <div
      v-if="!isEffective"
    ...
    <div class="mb-6 flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-ink-gray-5">
      ...
      <span v-if="doc.review_due" :class="reviewTone(doc.review_due) === 'red' ? 'text-ink-red-3' : ''">
        Review due {{ shortDate(doc.review_due) }}
      </span>
      <span v-for="tag in doc.tags || []" :key="tag" class="text-ink-gray-5">#{{ tag }}</span>
    </div>

    <!-- Mentions are rendered by MentionChip, which resolves them in one batched
         call; everything else is the stored content. -->
    <article ref="body" class="prose-sop text-base text-ink-gray-8" v-html="doc.content" />

    <MentionChip v-for="ref in doc.references || []" :key="ref.key" :reference="ref" :root="body" />
  ...
  <!-- Sticky on purpose: on a phone the operator must never hunt for this. -->
  <div
    v-if="needsAcknowledgement"
    class="sticky bottom-0 border-t bg-surface-base px-4 py-3 sm:px-6"
  >
